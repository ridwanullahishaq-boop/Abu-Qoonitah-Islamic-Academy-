...
export default function LMSPortal({ isArabic, currentUser, onLoginSuccess, onLogout, initialRegistering }: LMSPortalProps) {
  ...
  const [studentCertAccessMap, setStudentCertAccessMap] = useState<Record<string, "open" | "closed">>({});
...
Kindly verify my proof of payment and clear my academic lock. Jazakum Allahu Khairan!`;
  ...
  // Toggle individual student certificate access
  const handleToggleStudentCertificateAccess = async (studentId: string, currentCertAccess: string) => {
    const newStatus = currentCertAccess === "closed" ? "open" : "closed";
    const token = localStorage.getItem("token") || "";
    try {
      const res = await fetch(`/api/admin/students/${studentId}/certificate-access`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ certificateAccess: newStatus })
      });
      const data = await res.json();
      if (res.ok && data.success) {
        setAllStudents(prev => prev.map(s => s.id === studentId ? { ...s, certificateAccess: newStatus as any } : s));
        setStudentCertAccessMap(prev => ({ ...prev, [studentId]: newStatus as any }));
        setAllocSuccessMsg(`🎓 Certificate access updated to "${newStatus.toUpperCase()}"!`);
        setTimeout(() => setAllocSuccessMsg(""), 4000);
      }
    } catch (err: any) {
      console.error(err);
  ...
  // Bulk toggle certificate access for all students
  const handleBulkCertificateAccess = async (status: "open" | "closed") => {
    const token = localStorage.getItem("token") || "";
    try {
      const res = await fetch("/api/admin/students/bulk-certificate-access", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ certificateAccess: status })
      });
      const data = await res.json();
      if (res.ok && data.success) {
        setAllStudents(prev => prev.map(s => ({ ...s, certificateAccess: status })));
        const newMap: Record<string, "open" | "closed"> = {};
        allStudents.forEach(s => { newMap[s.id] = status; });
        setStudentCertAccessMap(newMap);
        setAllocSuccessMsg(`🎓 Graduation certificate access set to "${status.toUpperCase()}" for all ${data.count || allStudents.length} students!`);
        setTimeout(() => setAllocSuccessMsg(""), 4000);
      }
    } catch (err: any) {
      setAllocErrorMsg("Error setting bulk certificate access: " + err.message);
      setTimeout(() => setAllocErrorMsg(""), 4000);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-white dark:bg-natural-dark rounded-3xl p-6 border border-emerald-50 dark:border-emerald-900/40 shadow-sm flex flex-col items-center justify-center space-y-3">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-800 to-emerald-950 dark:from-emerald-900 dark:to-emerald-950 flex items-center justify-center border-2 border-natural-gold shadow-md shrink-0">
              <span className="text-natural-gold font-serif font-bold text-2xl tracking-tight">AQ</span>
            </div>
                ...
                <div className="space-y-6 animate-fade-in">
                            ...
                            <div className="lg:col-span-7 space-y-6">
                              ...
                              <div className="space-y-3 pt-4 border-t border-emerald-50 dark:border-emerald-850">
                                ...
                                <div className="space-y-2">
                                  ...
                                  {/* Active Quiz Box */}
                                  {activeQuiz && (
                                    <div id="active-cbt-box" className="bg-white dark:bg-emerald-950 rounded-lg p-4 border border-emerald-200 dark:border-emerald-800 space-y-4 text-xs animate-fade-in text-emerald-950 dark:text-white">
                                      ...
                                      {quizScore ? (
                                        <div className="space-y-4">
                                          <div className="text-center p-4 bg-emerald-50/60 dark:bg-emerald-900/60 rounded-xl border border-emerald-100 dark:border-emerald-800 space-y-2">
                                            <Award className="w-10 h-10 text-amber-500 mx-auto animate-bounce" />
                                            <div className="font-bold text-base text-emerald-900 dark:text-white">Exam Sheet Submitted & Evaluated!</div>
                                            {activeQuiz.automaticMarking === false ? (
                                              ...
                                              <>
                                                <div className="font-mono text-2xl font-black text-emerald-700 dark:text-amber-400">{quizScore.score}% Overall Score</div>
                                                <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium">Graded instantly in academic report cards.</p>
                                              </>
                                            )}
                                            <button 
                                              onClick={() => { setActiveQuiz(null); setQuizScore(null); }} 
                                              className="px-4 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-lg text-xs cursor-pointer shadow-xs transition-all mt-2"
                                            >
                                              Close Correction View
                                            </button>
                                          </div>

                                          {/* Question-by-Question Answer Sheet & Corrections */}
                                          <div className="space-y-3 pt-2">
                                            <h6 className="font-serif font-bold text-xs uppercase tracking-wider text-amber-700 dark:text-amber-300 border-b border-emerald-100 dark:border-emerald-800 pb-2 flex items-center justify-between">
                                              <span>📝 Your Answer Sheet & Correction Breakdown</span>
                                              <span className="text-[10px] font-mono text-slate-400 font-normal">
                                                {activeQuiz.questions ? activeQuiz.questions.length : 0} Questions Total
                                              </span>
                                            </h6>

                                            <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
                                              {(activeQuiz.questions || []).map((q: any, idx: number) => {
                                                const studentAnsIdx = quizAnswers[q.id];
                                                const correctAnsIdx = q.correctAnswerIndex !== undefined ? q.correctAnswerIndex : q.correctIndex;
                                                ...
                                                return (
                                                  ...
                                                  >
                                                    <div className="flex items-start justify-between gap-2">
                                                      <span className="font-bold text-emerald-950 dark:text-amber-100 flex-1">
                                                        <span className="font-mono text-amber-600 dark:text-amber-400 mr-1.5">Q{idx + 1}.</span>
                                                        {q.questionText}
                                                      </span>
                                                    ...
                                                    <div className="grid grid-cols-1 gap-1.5 pl-2">
                                                      {(q.options || []).map((opt: string, optIdx: number) => {
                                                        ...
                                                        let optBadge = null;
                                                        let optStyle = "bg-white dark:bg-emerald-900/30 border-slate-200 dark:border-emerald-800/50 text-slate-700 dark:text-slate-300";

                                                        if (isSelected && isCorrectOpt) {
                                                          optStyle = "bg-emerald-100 dark:bg-emerald-900/80 border-emerald-500 text-emerald-950 dark:text-emerald-100 font-bold ring-1 ring-emerald-500";
                                                          optBadge = <span className="text-[9px] bg-emerald-600 text-white font-mono px-2 py-0.5 rounded font-bold">Your Choice (Correct ✓)</span>;
                                                        } else if (isSelected && !isCorrectOpt) {
                                                          optStyle = "bg-red-100 dark:bg-red-950/80 border-red-400 text-red-950 dark:text-red-200 font-bold ring-1 ring-red-400";
                                                          optBadge = <span className="text-[9px] bg-red-600 text-white font-mono px-2 py-0.5 rounded font-bold">Your Choice (Incorrect ✕)</span>;
                                                        } else if (isCorrectOpt && !isCorrect) {
                                                          optStyle = "bg-amber-100/90 dark:bg-amber-950/70 border-amber-400 text-amber-950 dark:text-amber-200 font-bold ring-1 ring-amber-400";
                                                          optBadge = <span className="text-[9px] bg-amber-500 text-emerald-950 font-mono px-2 py-0.5 rounded font-extrabold">Official Correct Answer ✓</span>;
                                                        }

                                                        return (
                                                          <div key={optIdx} className={`p-2 rounded-lg border text-xs flex items-center justify-between gap-2 ${optStyle}`}>
                                                            <span>
                                                              <strong className="font-mono text-slate-400 mr-1.5">{String.fromCharCode(65 + optIdx)}.</strong>
                                                              {opt}
                                                            </span>
                                                            {optBadge}
                                                          </div>
                                                    ...
                                                    {/* Correction Note */}
                                                    {!isCorrect && correctAnsIdx !== undefined && q.options && q.options[correctAnsIdx] && (
                                                      <div className="bg-amber-500/10 dark:bg-amber-950/30 border border-amber-500/30 rounded-lg p-2.5 text-[11px] text-amber-900 dark:text-amber-200 font-sans space-y-0.5">
                                                        <div className="font-bold flex items-center gap-1 text-amber-700 dark:text-amber-300">
                                                          💡 Correction & Academic Key:
                                                        </div>
                                                        <div>
                                                          The correct answer is <strong>Option {String.fromCharCode(65 + correctAnsIdx)}: {q.options[correctAnsIdx]}</strong>.
                                                        </div>
                                                      </div>
                                                    )}
                                                  </div>
                  ...
                  {studentSubTab === "certificates" && (
                    <div className="space-y-6">
                      ...
                      {currentUser.certificateAccess === "closed" ? (
                        <div className="bg-amber-500/10 border-2 border-amber-500/60 rounded-2xl p-6 sm:p-8 text-center space-y-4 dark:bg-amber-950/40 shadow-xs animate-fade-in">
                          <div className="w-16 h-16 bg-amber-400 text-emerald-950 rounded-full flex items-center justify-center mx-auto text-2xl font-bold shadow-sm">
                            🔒
                          </div>
                          <div className="space-y-1">
                            <h3 className="text-xl font-serif font-bold text-amber-900 dark:text-amber-200">
                              Graduation Certificate Access Closed by Administration
                            </h3>
                            <p className="text-xs text-amber-800/90 dark:text-amber-300/90 max-w-xl mx-auto leading-relaxed">
                              Your graduation certificate download portal is currently <strong>CLOSED (Locked)</strong> by Abu Qoonitah Academy Administration. Certificate downloads are controlled to ensure academic review, tuition clearance, or scheduled graduation date releases.
                            </p>
                          </div>
                          <div className="pt-2 flex justify-center">
                            <span className="text-[11px] font-mono bg-amber-200 dark:bg-amber-900 text-amber-950 dark:text-amber-100 px-4 py-1.5 rounded-full font-bold border border-amber-400/50 flex items-center gap-2">
                              <Lock className="w-3.5 h-3.5 text-amber-700 dark:text-amber-300" />
                              <span>Status: Access Closed by Admin (Contact Ustadh or Admin)</span>
                            </span>
                          </div>
                        </div>
                      ) : (
                    ...
                    <div className="bg-white dark:bg-emerald-900 rounded-xl p-6 sm:p-8 border border-emerald-100 dark:border-emerald-800 shadow-sm space-y-6 font-sans">
                      ...
                      {(allocRoleFilter === "all" || allocRoleFilter === "students") && (
                        <div className="space-y-4 pt-4 border-t border-emerald-100 dark:border-emerald-800">
                          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-2 border-b border-emerald-100 dark:border-emerald-800 pb-2">
                              <div className="flex items-center gap-2">
                                <h4 className="font-serif font-bold text-emerald-950 dark:text-amber-100 text-base flex items-center gap-2">
                                  <span>🎓 Registered Students Level, Teacher Allocation & Certificate Controls</span>
                                </h4>
                              ...
                              {/* Admin Bulk Certificate Controls */}
                              <div className="flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() => handleBulkCertificateAccess("open")}
                                  className="px-3 py-1.5 bg-emerald-800 hover:bg-emerald-900 text-amber-300 text-[11px] font-bold rounded-lg cursor-pointer transition-all border border-amber-400/40 flex items-center gap-1 shadow-xs"
                                  title="Open certificate downloads for all registered students"
                                >
                                  <Award className="w-3.5 h-3.5 text-amber-400" />
                                  <span>Open All Certificates</span>
                                </button>
                                <button
                                  type="button"
                                  onClick={() => handleBulkCertificateAccess("closed")}
                                  className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-emerald-950 text-[11px] font-bold rounded-lg cursor-pointer transition-all border border-amber-600/40 flex items-center gap-1 shadow-xs"
                                  title="Close/Lock certificate downloads for all registered students"
                                >
                                  <Lock className="w-3.5 h-3.5 text-emerald-950" />
                                  <span>Close All Certificates</span>
                                </button>
                              </div>
                            </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                  <thead>
                                    <tr className="bg-slate-50 dark:bg-emerald-950 border-b border-emerald-100 dark:border-emerald-800 text-emerald-950 dark:text-amber-100 font-bold uppercase tracking-wider text-[10px]">
                                      ...
                                      <th className="p-3.5">Certificate Access</th>
                                      <th className="p-3.5 pr-4 text-right">Action</th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y divide-emerald-50/15">
                                    {allStudents
                                      ...
                                      .map((student) => {
                                        ...
                                        const currentCertAccess = studentCertAccessMap[student.id] ?? (student.certificateAccess || "open");
                                          ...
                                          <tr key={student.id} className="hover:bg-slate-50/50 dark:hover:bg-emerald-850/20 transition-colors">
                                            ...
                                            <td className="p-3.5">
                                              <button
                                                type="button"
                                                onClick={() => {
                                                  const nextStatus = currentCertAccess === "closed" ? "open" : "closed";
                                                  setStudentCertAccessMap({ ...studentCertAccessMap, [student.id]: nextStatus });
                                                  handleToggleStudentCertificateAccess(student.id, currentCertAccess);
                                                }}
                                                className={`px-2.5 py-1 rounded-full text-[10px] font-bold cursor-pointer transition-all flex items-center gap-1 ${
                                                  currentCertAccess === "open"
                                                    ? "bg-emerald-100 text-emerald-800 border border-emerald-200 dark:bg-emerald-900/60 dark:text-emerald-200"
                                                    : "bg-amber-100 text-amber-900 border border-amber-300 dark:bg-amber-950/80 dark:text-amber-200"
                                                }`}
                                              >
                                                {currentCertAccess === "open" ? (
                                                  <>
                                                    <Award className="w-3 h-3 text-emerald-600 dark:text-emerald-300" />
                                                    <span>Cert Open (Allowed)</span>
                                                  </>
                                                ) : (
                                                  <>
                                                    <Lock className="w-3 h-3 text-amber-600 dark:text-amber-400" />
                                                    <span>Cert Closed (Locked)</span>
                                                  </>
                                                )}
                                              </button>
                                            </td>
